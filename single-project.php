...
******************************************************/
	...
	if(isset($_POST['bid_now_reverse']))
	{
		...
		{
			$pid 		= base64_decode(projecttheme_sanitize_string($_POST['control_id']));
			$post 		= get_post($pid);
			$bid 		= trim($_POST['bid']);
...
<?php
    ...
    <div class="my_box3">
            <div class="padd10">
                ...
                <?php echo sprintf(__('The project "%s" was marked as hidden. <a href="%s">Please login</a> to see project details.','ProjectTheme') , $post->post_title, get_bloginfo('url')."/wp-login.php"); ?>
                </div>
    </div>
    </div>
...
?>
            ...
            <div class="my_box3">
				...
				if(count($res) > 0)
				{
					...
					foreach($res as $row)
					{
						...
						if ($owner == 1 ) {
							...
							$nr = 7;
							if(empty($winner)) // == 0)
								echo '<div><i class="bid-select"></i>  <a href="'.get_bloginfo('url').'/?p_action=choose_winner&pid='.get_the_ID().'&bid='.$row->id.'">'.__('Select as Winner','ProjectTheme').'</a></div>';
				...
				else { echo '<div class="padd10">'; _e("No proposals placed yet.",'ProjectTheme'); echo '</div>'; }
				?>
		...
		?>
			...
			<div class="my_box3">
                ...
                <script type="text/javascript" src="http://maps.google.com/maps/api/js?key=<?php echo get_option('ProjectTheme_radius_maps_api_key') ?>&sensor=false"></script>
       ...
       <p id='proposal_btn_p'>
        ...
        <div class="avatar-op-wrap">
        	...
        	<div class="avatar-op-list">
            		<ul>
                    	<li><a class="avatar-posted-by-username" href="<?php bloginfo('url'); ?>/?p_action=user_profile&post_author=<?php echo $post->post_author; ?>"><?php the_author() ?></a></li>
        ...
        <ul class="other-dets other-dets2">
            	...
            	<br/><br/>
               <a href="<?php bloginfo('url'); ?>/?p_action=user_profile&post_author=<?php echo $post->post_author; ?>"><?php _e('See More Projects by this user','ProjectTheme'); ?></a><br/>
