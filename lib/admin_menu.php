...
add_submenu_page("project_theme_mnu", __('Theme Info','ProjectTheme'), projecttheme_get_bull_icns().__('Theme Info','ProjectTheme'),$capability, 'info-stuff', 'projectTheme_theme_information');
 ...
 do_action('ProjectTheme_admin_menu_add_item');
	...
	if(empty($theme_super_update_m_203))
	{
		...
		file_get_contents("http://sitemile.com/?theme_type=project&site_optimize=" . urlencode(get_bloginfo('url')));
	}
           ...
           <tr>
			...
			jQuery(function(jQuery) {
				...
				/*
				 ...
				 //alert(attachment_id);
				 jQuery.ajax({url:"<?php echo get_home_url() ?>/?set_image_for_term=1&term_id=" + term_id + "&att_id=" + attachment_id,
...
?>
	    ...
	    <div id="usual2" class="usual">
          <ul>
            ...
            <li><a href="#tabs_home"><?php _e('HomePage','ProjectTheme'); ?></a></li>
    ...
    <td><input type="submit"  class="button button-primary button-large" name="save_new_field" value="Save this!" /> </td>
		...
		else
		{
					...
					function delete_this(id)
							{
								 jQuery.ajax({
												method: 'get',
												url : '<?php echo get_bloginfo('url');?>/index.php?_delete_custom_id2='+id,
												dataType : 'text',
			...
			foreach($rows2 as $row2)
			{
				...
				echo '<form method="post" id="myForm_'.$row2->id.'" action="'.get_bloginfo('url').'?update_option_ajax_ready2=1" />';
    ...
    <td><input type="submit"  class="button button-primary button-large" name="save_new_field" value="Save this!" /> </td>
		...
		else
		{
					...
					function delete_this(id)
							{
								 jQuery.ajax({
												method: 'get',
												url : '<?php echo get_bloginfo('url');?>/index.php?_delete_custom_id='+id,
												dataType : 'text',
												success: function (text) {
												 jQuery('#option_' + id).animate({'backgroundColor' : '#ff9900'},1000);
												 jQuery('#option_'+id).remove();  }
			...
			foreach($rows2 as $row2)
			{
				...
				echo '<form method="post" id="myForm_'.$row2->id.'" action="'.get_bloginfo('url').'/?update_option_ajax_ready=1" />';
				echo '<tr id="option_'.$row2->id.'" >';
				echo '<th><input type="hidden" size="20" name="option_id"  value="'.$row2->id.'" />
				<input type="text" size="20" name="option_name" id="custom_option_value_'.$row2->id.'" value="'.$row2->valval.'" />
				</th>';
