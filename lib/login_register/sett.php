...
function ProjectTheme_new_user_notification($user_id, $plaintext_pass = '') {
	...
	$replace 	= array($user_login, $user_email, $site_login_url, $site_name, get_bloginfo('url'), $account_url, $plaintext_pass);
...
function my_retrieve_password()
{
	...
	$message .= get_bloginfo('url') . "<br/><br/>";
	$message .= sprintf(__('Username: %s', $current_theme_locale_name), $user_login) . "<br/>";
	$message .= __('If this was a mistake, just ignore this email and nothing will happen.', $current_theme_locale_name) . "<br/>";
	$message .= __('To reset your password, visit the following address:' ,$current_theme_locale_name) . "<br/>";
	$message .= '' . get_bloginfo('url')."/wp-login.php?action=rp&key=$key&login=" . rawurlencode($user_login) . "<br/>";
