		...
		<!-- page content here -->	
				...
				if(count($r) > 0)
				{
					...
					foreach($r as $row)
					{
						...
						echo '<tr>';
						echo '<td class="'.$cls.'"><a href="'.get_bloginfo('url').'/?p_action=user_profile&post_author='.$user->ID.'">'.$user->user_login.'</a></td>';
		...
		<!-- page content here -->	
				...
				if(count($r) > 0)
				{
					...
					echo '<tr><td colspan="5">  ';
						 ...
						 echo ProjectTheme_get_my_pagination_main(get_bloginfo('url'). "/?page_id=".get_option('ProjectTheme_my_account_private_messages_id'), 
