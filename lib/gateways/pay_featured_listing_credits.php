<?php
	...
	else $bklnk = get_bloginfo('url')."/?page_id=".get_option('PricerrTheme_pay_for_posting_job_page_id')."&jobid=".$pid;
	
	//------------------------------------------------------------------------------------------------------------------------------
	
	echo '<a class="payment_feat" href="'.get_bloginfo('url').'/?jb_action=pay_featured_credits&confirm=yes&jobid='.$pid.'">'. __('Confirm Payment','PricerrTheme').'</a> ';
